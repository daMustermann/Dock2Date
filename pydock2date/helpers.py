...
def run_hook(
    hookname: str, myglobals: dict | None = None, mylocals: dict | None = None
):
    """
    ...
    try:
        ...
        getLogger().error(
            "An error was raised while scanning the directory for hook %s",
            hookname,
            exc_info=True,
        )
        return
...
def execfile(
    filepath: str, myglobals: dict | None = None, mylocals: dict | None = None
):
    """
    ...
    try:
        with open(filepath, "rb") as file:
            try:
                ...
                getLogger().error(
                    "An error was raised while executing hook script %s",
                    filepath,
                    exc_info=True,
                )
    except Exception:
        getLogger().error(
            "An error was raised while reading hook script %s", filepath, exc_info=True
        )
...
def set_properties(old, new, self_name: str | None = None) -> dict:
    ...
    properties = {
        ...
        "ports": (
            None
            if (isContainerNetwork(old) or not old.attrs["Config"].get("ExposedPorts"))
            else [
