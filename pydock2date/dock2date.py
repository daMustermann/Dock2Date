...
from pydock2date.logger import Dock2DateLogger
...
def main():
    ...
    parser = ArgumentParser(
        description="Dock2Date",
        formatter_class=RawTextHelpFormatter,
        epilog="EXAMPLE: Dock2Date -d tcp://1.2.3.4:5678 -i 20 -m container1 container2 -l warn",
    )
    ...
    core_group.add_argument(
        ...
        dest="SELF_UPDATE",
        action="store_true",
        help="Let Dock2Date update itself",
    )
    ...
    docker_group.add_argument(
        ...
        dest="SWARM",
        action="store_true",
        help="Put Dock2Date in swarm mode",
    )
    ...
    docker_group.add_argument(
        ...
        dest="LABEL_ENABLE",
        action="store_true",
        help="Enable label monitoring for Dock2Date label options\n"
    ...
    data_group.add_argument(
        "-sc",
        "--save-counters",
        default=Config.save_counters,
        dest="SAVE_COUNTERS",
        action="store_true",
        help="Save total-updated counters across self-updates",
    )
    ...
    docker_group.add_argument(
        ...
        action="store_true",
        help="Do not send Dock2Date notifications when starting",
    )
    ...
    try:
        language = gettext.translation(
            "Dock2Date", localedir="locales", languages=args.LANGUAGE
        )
        language.install()
        _ = language.gettext
    ...
    ol = Dock2DateLogger(level=log_level)
    ...
    ol.logger.debug(_("Dock2Date configuration: %s"), config_dict)
    ...
    for socket in config.docker_sockets:
        try:
            ...
            if config.run_once:
                scheduler.add_job(
                    mode.update, name=_("Run Once container update for %s") % socket
                )
            else:
                if mode.mode == "container":
                    scheduler.add_job(
                        mode.self_check, name=_("Self Check for %s") % socket
                    )
                if config.cron:
                    scheduler.add_job(
                        mode.update,
